...
Перейдите в директорию shared (Не уверен, что надо так подробно расписывать, но пусть):
#+begin_src shell
cd shared
#+end_src

Склонируйте в shared репозиторий (git у вас тоже должен быть и не забудьте точку в конце команды):
#+begin_src shell
git clone https://www.kgeorgiy.info/git/geo/java-advanced-2025.git .
#+end_src

Перейдите в директорию ~solutions~ и склонируйте ваш персональный репозиторий (Тоже точка в конце команды):
#+begin_src shell
cd ..
cd solutions
...
Из кода несложно понять, что мы подтягиваем зависимости, указываем директорию исходников.
...
* Заключение
Я повторил все шаги с нуля, у меня всё работает. Прежде чем идти на сдачу, убедитесь, что вы понимаете как всё работает. Я всего лишь поделился тем, как собираю проект я.
